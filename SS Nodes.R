...
MakeLink=function(fromID,LinkName, toID){
    # (from) -[hasTo]- (to)
    N=length(fromID);
    LinkCode=LinkType$LinkName
    if(is.na(linkCode)){cat"MakeLink, bad LinkName =",LinkName,"\n"),
         LinkCode="00",LinkName="has"} # bad LinkName reverts to default
    id=MakeID(N,LinkCode)
    # build 6 character Edge code, 010203 is From 01 To 02 By 03
    From=substring(fromID,1,2); To=substring(toID,1,2);
    id=paste0(From,To,id) # e.g. "010201654321"
    # if just "has" expand to "hasSomething"
    if(LinkName=="has") LinkName=paste0(LinkName,)
    a=data.frame(fromID,LinkName,toID,id=id,description="",
         status="active", startDate="",endDate="", stringsAsFactors = FALSE);
    for(j in 1:N) createRel()
    # (:Flight {number:{flight_num}})-[:DESTINATION]->(destination)
    "
